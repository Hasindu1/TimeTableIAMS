<!DOCTYPE html>
<%@page import="iams.modal.Modules"%>
<%@page import="java.util.ArrayList"%>
<%@page import="iams.dao.ModuleDao"%>
<html>
<head>
...
<body>
	...
	<div class="wrapper">
		...
		<div class="form-popup" id="myForm">
			<form class="form-container" name="signupform" action="ModuleServelet"
				method="post">
				<h1>Add Module</h1>

				<label for="modulecode"><b>Module Code</b></label> <input type="text"
					placeholder="Enter Module Code" name="modulecode" required>
					
				 <label for="moduleName"><b>Module Name</b></label> <input type="text"
					placeholder="Enter Module Name" name="moduleName" required> 
					
				
				
				<label for="academicYearSem"><b>Academic Year and Semester</b></label> <select class="form-control"
					name="academicYearSem" required>
					<option>Y1.S1</option>
					<option>Y1.S2</option>
					<option>Y2.S1</option>
					<option>Y2.S2</option>
				...
				<label for="lectureIncharge"><b>Lecture In Charge</b></label> <input type="text"
					placeholder="Enter Lecture In Charge" name="lectureIncharge" required>
				...
				<table id="table" data-toggle="table" data-search="true"
					...
					<thead>
						<tr>
							...
							<th data-field="modulecode" data-filter-control="input"
								data-sortable="true">Module Code</th>
							<th data-field="moduleName" data-filter-control="input"
								data-sortable="true">Module Name</th>
							<th data-field="academicYearSem" data-filter-control="input"
								data-sortable="true">Academic Year and Semester</th>
							<th data-field="lectureIncharge" data-filter-control="input"
								data-sortable="true">Lecture In Charge</th>
					...
					<%
						...
						ArrayList<Modules> modules = module.getModules();

						for (Modules h : modules) {
					%>
					<tr>


						<td><%=h.getModulecode()%></td>
						<td><%=h.getModuleName()%></td>
						<td><%=h.getAcademicYearSem()%></td>
						<td><%=h.getLectureIncharge()%></td>
						
						
						<form method="POST" action="EditHall">
						<td><button class="btn btn-info" type="submit" name="btn_edit" value=<%=h.getModulecode()%>>Edit</button></td>
						</form>
						
						<form method="POST" action="DeleteModuleServelet">
							<td><button type="submit" class="btn btn-danger"
									name="btn_delete" value=<%=h.getModulecode()%>>Delete</button></td>
						</form>
